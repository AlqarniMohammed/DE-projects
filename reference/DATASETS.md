...
Every dataset the default path uses, plus the vetted menu options for satellite variation (see [`prompts/generate-satellite-requirements.md`](../prompts/generate-satellite-requirements.md)). Standing rules: datasets never enter git — your platform repo's `.gitignore` must cover `data/` (this repo ships the template block); re-verify size/availability before a phase starts; **default** marks what the default path uses.

## The platform's datasets (carried across the year)

| Dataset | Used in | Size | Source | Why chosen |
|---|---|---|---|---|
| **NYC TLC yellow-taxi trips** (Parquet, monthly) — *default* | P0 first contact · P1–P6 platform, all layers | ~50–60 MB/month; take 6+ months | [nyc.gov TLC trip records](https://www.nyc.gov/site/tlc/about/tlc-trip-record-data.page) | The proven lakehouse teaching dataset: real volume, real warts (fare outliers, late corrections), monthly cadence for incremental/backfill drills, already Parquet |
| **Taxi-zone lookup** (CSV) | P1 platform — the join/enrichment dimension, later the SCD2 snapshot | ~12 KB | Same TLC page | The dimension table: joins, `dim_zones`, SCD2 practice |
| **Weather API** (e.g., Open-Meteo) | P4 platform — the dlt REST source enriching taxi demand | API, incremental | [open-meteo.com](https://open-meteo.com/) | Free, keyless, incremental-cursor-friendly — ideal dlt teaching source |
...
|---|---|---|---|
| **Hacker News dump** — *default* | ~10 GB full; sample fine | [Kaggle/BigQuery mirrors](https://www.kaggle.com/datasets/hacker-news/hacker-news) (bulk) · [official HN API](https://github.com/HackerNews/API) (per-item only — not practical for a 10 GB pull) | Proven; stories/comments/users = natural 3-layer modeling |
...
| Dataset | Size | Source | Notes |
|---|---|---|---|
| **SEC EDGAR financial statement sets** — *default* | 5–10 GB selectable | [sec.gov EDGAR](https://www.sec.gov/dera/data/financial-statement-data-sets.html) | Quarterly files → partitioning story writes itself. SEC requires a declared User-Agent header with contact info on automated requests (403 without it) — a real-world ingestion lesson |
...
Both run on **your own platform data** by design (S4a adds one small native table from any menu above); the comparison satellite rebuilds an existing platform pipeline — no new dataset.

### S5 — CDC showdown (P5) — runs on the seeded OLTP generator
| Dataset | Size | Source | Notes |
|---|---|---|---|
| **Bookings app** — *default* | generated, small | you build the generator (spec in the P4 README) | Same schema and generator as P4's DMS leg — that identity is the point of the showdown |
| E-commerce orders & returns | generated, small | you build the generator (spec in the P4 README) | Alternative schema flavor via the requirements generator |
| Clinic appointments | generated, small | you build the generator (spec in the P4 README) | Alternative schema flavor via the requirements generator |

### S6a — Serving layer (P6)
| Dataset | Size | Source | Notes |
|---|---|---|---|
| **Your own gold marts** — *default* | — | the platform | The point is the hot-copy pattern, not new data |
| Tadawul (Saudi Exchange) market summaries | small | [saudiexchange.sa](https://www.saudiexchange.sa/) | Regional differentiator for the dashboard/data-product story; if saudiexchange.sa is unreachable from your region, fall back to an [open.data.gov.sa](https://open.data.gov.sa/) financial dataset |
